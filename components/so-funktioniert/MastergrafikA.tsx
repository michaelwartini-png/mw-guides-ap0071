 ...
 * Website-Einbindung von Mastergrafik A V2.3 ohne Grafik-Kopf und
 * Grafik-Fuß. Original bleibt v2.3.html.
 * Siehe docs/AP-G-so-funktioniert-v1.0.md — Änderungen nur als 1.1+.
 */
export function MastergrafikA() {
  const wrapRef = useRef<HTMLDivElement>(null);
  const frameRef = useRef<HTMLIFrameElement>(null);
  const [scale, setScale] = useState(1);
  const [graphicHeight, setGraphicHeight] = useState(1100);
  ...
  return (
    ...
    >
      <iframe
        ref={frameRef}
        title="Mastergrafik A — So funktioniert MW Guides"
        src="/mastergrafik-a/v2.3-embed.html"
        width={GRAPHIC_WIDTH}
