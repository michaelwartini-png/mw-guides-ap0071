...
import { getReiseplanAufgaben } from "@/content/reiseplanAufgaben";

interface MeineReiseSidebarProps {
  totalHighlights: number;
}

/** AP-011 / AP-ET003 — selected highlights, optional follow-up tasks, progress. */
        ...
        {items.length === 0 ? (
          ...
          <ul className="divide-y divide-[var(--mwg-line)]">
            {items.map((item, index) => {
              const aufgaben = getReiseplanAufgaben(item.slug);
              return (
                        ...
                        aufgabe.external ? (
                          <li key={aufgabe.id}>
                            <a
                              ...
                              className="inline-flex items-center gap-1 text-[12px] text-[var(--mwg-accent)] hover:underline"
                            >
                              {aufgabe.label}
                              {aufgabe.optional ? " (optional)" : ""}
                              <ArrowRight size={11} />
                            </a>
                          </li>
                        ) : (
